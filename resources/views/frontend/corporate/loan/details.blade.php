...
@section('content')
    <div class="row">
        <div class="col-xl-12 col-lg-12 col-md-12 col-12">
                ...
                <div class="site-card-header">
                    ...
                    @if ($loan->status == App\Enums\LoanStatus::Reviewing)
                        <div class="card-header-links d-flex">
                            <a href="#" data-id="{{ $loan->loan_no }}" class="bg-danger card-header-link cancelBtn">
                                <i data-lucide="x"></i>
                                {{ __('Cancel') }}
                            </a>
                        </div>
                    ...
                    <div class="site-custom-table site-custom-table-sm">
                        <div class="contents">
                                ...
                                <div class="site-table-col">
                                    <div class="fw-bold">
                                        @if ($loan->status->value == 'running')
                                            <div class="type site-badge badge-primary">{{ ucfirst($loan->status->value) }}
                                            </div>
                                        @elseif($loan->status->value == 'rejected' || $loan->status->value == 'cancelled')
                                            <div class="type site-badge badge-failed">{{ ucfirst($loan->status->value) }}
                                            </div>
                                        @elseif($loan->status->value == 'completed')
                                            <div class="type site-badge badge-success">{{ ucfirst($loan->status->value) }}
                                            </div>
                                        @elseif($loan->status->value == 'due')
                                            <div class="type site-badge badge-failed">{{ ucfirst($loan->status->value) }}
                                            </div>
                                        @elseif($loan->status->value == 'reviewing')
                                            <div class="type site-badge badge-pending">{{ ucfirst($loan->status->value) }}
                                            </div>
                                        @endif
                            ...
                            <div class="site-table-list">
                                ...
                                <div class="site-table-col">
                                    <div class="fw-bold">
                                        <div class="fw-bold">{{ $loan->perInstallment() . ' ' . $currency }} (Every
                                            {{ $loan->plan->installment_intervel }} Days)</div>
                                    </div>
                                </div>
                            </div>
                            <div class="site-table-list">
                            ...
                            <div class="site-table-list">
                                ...
                                <div class="site-table-col">
                                    <div class="fw-bold"><span
                                            class="type site-badge badge-primary">{{ $loan->givenInstallemnt() ?? 0 }}</span>
                                    </div>
                                </div>
                            </div>
                            <div class="site-table-list">
                            ...
                            <div class="site-table-list">
                                ...
                                <div class="site-table-col">
                                    <div class="red-color fw-bold">{{ $loan->plan->charge }}
                                        {{ $loan->plan->charge_type == 'percentage' ? '%' : $currency }} /
                                        {{ $loan->plan->delay_days }} Day</div>
                                </div>
                            </div>
                            <div class="site-table-list">
                                <div class="site-table-col">
            ...
            @if ($loan->transactions->count() > 0)
                <div class="site-card">
                    <div class="site-card-header">
                        ...
                        @if ($loan->status == App\Enums\LoanStatus::Running || $loan->status == App\Enums\LoanStatus::Due)
                            <div class="card-header-links d-flex">
                                <button type="button"
                                    data-url="{{ route('user.loan.pay.installment', ['loan_id' => encrypt($loan->id)]) }}"
                                    class="site-btn-sm polis-btn payLoanInstallment">
                                    ...
                                    <div class="site-table-list">
                                        ...
                                        <div class="site-table-col text-center">
                                            @if ($trx->given_date == null)
                                                <button type="button"
                                                    data-url="{{ route('user.loan.pay.installment', ['loan_id' => encrypt($loan->id), 'trans_id' => encrypt($trx->id)]) }}"
                                                    class="site-btn-sm polis-btn payLoanInstallment">
                                                    <i data-lucide="send"></i>
                                                    {{ __('Pay Installment') }}
                                                </button>
            ...
            @if ($loan->status == App\Enums\LoanStatus::Reviewing)
                ...
                <div class="modal fade" id="cancelLoan" tabindex="-1" aria-labelledby="cancelLoanModalLabel"
                    aria-hidden="true">
            ...
            <div class="modal fade" id="payLoanInstallmentModal" tabindex="-1" aria-labelledby="payloanModalLabel"
                aria-hidden="true">
                <div class="modal-dialog modal-md modal-dialog-centered">
                    <div class="modal-content site-table-modal">
                        <div class="modal-body popup-body">
    ...
    @push('js')
        <script>
            ...
            $(document).on('click', '.payLoanInstallment', function(e) {
                var url = $(this).data('url');
                $('.confirm_pay_installment_btn').attr('href', url);

                $('#payLoanInstallmentModal').modal('show');
            });
        </script>
    @endpush
@endsection
