       ...
       <div class="auth-main-grid">
          <div class="auth-main-from">
             <div class="auth-from-inner">
                ...
                <div class="auth-from-box">
                    <form id="otp-form" action="{{ route('email.otp.verify.post') }}" method="POST">
                        @csrf

                       <div class="row gy-24">
                        <div class="col-xxl-12">
                            <div class="otp-code-status">
                               <span class="title">{{ __('Enter the 6-digit OTP code sent to') }}
                                    <strong>{{ auth()->user()->email }}</strong>
                                </span>
                            </div>
                            @if(session('success'))
                                <div class="alert alert-outline td-alert-success alert-outline d-flex gap-2 align-items-center alert-dismissible fade show mt-3" role="alert">
                            ...
                            @endif
                            @if(session('error'))
                                <div class="alert alert-outline td-alert-danger alert-outline d-flex gap-2 align-items-center alert-dismissible fade show mt-3" role="alert">
                      ...
                      <div class="auth-bottom-content mt-35">
                        ...
                        <div class="auth-account">
                            <p class="description">{{ __('Didn\'t receive the code?') }} <a class="link" href="{{ route('email.otp.resend') }}">{{ __('Resend OTP') }}</a></p>
                         </div>
                      </div>
                   </form>
                </div>
          ...
          <div class="auth-thumb-wrapper">
             <div class="auth-otp-thumb">
                <img src="{{ asset(getPageSetting('breadcrumb')) }}" alt="email-verification">
             </div>
          </div>
       </div>
    </div>
