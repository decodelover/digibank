...
if (! function_exists('getLocation')) {
    ...
    {
        // In local/development environment, skip API call and return default
        ...
        try {
            ...
            $locationData = curl_get_file_contents('http://ip-api.com/json/'.$ip);
            
            // Check if curl returned false
            if (!$locationData) {
                return new \Illuminate\Support\Fluent([
                    'country_code' => 'US',
                    'name' => 'United States',
            ...
            // Check if location data exists and has countryCode
            if (! is_array($location) || ! isset($location['countryCode'])) {
                return new \Illuminate\Support\Fluent([
                    'country_code' => 'US',
                    'name' => 'United States',
                    'dial_code' => '+1',
            ...
            // Return default country if any error occurs
            return new \Illuminate\Support\Fluent([
                'country_code' => 'US',
                'name' => 'United States',
                'dial_code' => '+1',
...
if (! function_exists('sanitizeAccountNumber')) {
    function sanitizeAccountNumber($account_number)
    {
        if (Str::startsWith($account_number, setting('account_number_prefix', 'global'))) {
            return trim(Str::replaceFirst(setting('account_number_prefix', 'global'), '', $account_number));
        }
...
if (! function_exists('branch_enabled')) {
    ...
    {
        return (bool) setting('multi_branch');
    }
}

if (! function_exists('transaction_currency')) {
    function transaction_currency($transaction)
    {
