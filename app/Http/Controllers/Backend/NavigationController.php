...
class NavigationController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            'id' => 'required',
            'name' => 'required',
            'select_page' => 'required_without:custom_url',
            'custom_url' => 'required_if:select_page,custom|required_if:select_page,',
            'type' => new Enum(NavigationType::class),
            'status' => 'required',
        ], [
            'select_page.required_without' => __('Please select a page or enter a custom URL.'),
            'custom_url.required_if' => __('Custom URL is required when using custom page type.'),
        ]);
        ...
        // Handle custom URL or empty select_page
        if (empty($pageId) || $pageId == 'custom') {
            $pageId = null;
            // Use custom_url if provided
            $url = $input['custom_url'] ?? $url;
        } else {
            $page = Page::find($input['select_page']);
            if ($page) {
                $url = $page->url;
